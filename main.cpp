...
void line(TGAImage &image, int **buffer,
...
void rendu(TGAImage &image, Model model)
{
    ...
    for(unsigned int i = 0 ; i < model.faces.size() ; i++)
    {
        ...
        z1 = (int)(model.sommets[face[0][0]-1].z*zw) * 1024;
        z2 = (int)(model.sommets[face[1][0]-1].z*zw) * 1024;
        z3 = (int)(model.sommets[face[2][0]-1].z*zw) * 1024;
